...
The p-value (1.23e-06) was small, so we reject the null.
...
Yes, there is a link between politics and TikTok use. The data shows that TikTok usage isn’t evenly spread across political groups, and some groups use it more than others.
...
The p-value is again small, so we reject the null. 
...
Yes, the device type seems to be related to how many platforms someone uses. With the scores, smartphone users had a higher average than PC users, suggesting smartphone users are on social media platforms more overall.
...
Yes, the overall ANOVA was significant. Based on the Tukey, the only group difference that was actually significant was Democrats and Republicans. The other pairs weren’t statistically different I believe.
